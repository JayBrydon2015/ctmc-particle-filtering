...
l = 4 # SE Kernel parameter
n = 3 # Number of states in CTMC
J = 1000 # Number of random walkers
mu0  = np.array([1, 0.1, 0.1, 1, 4, 0.1]) # For P(A_0), a vague prior
scale0 = np.array([0.3, 0.09, 0.09, 0.3, 0.5, 0.09]) # For P(A_0), a vague prior
...
if J <= 10:
    ...
    if SAVE_PLOTS:
        folder_path = Path(PLOT_ROOT_FOLDER_NAME + "/"
                           + f"{EXAMPLE_FOLDER_NAME}/GP; J={J}")
        folder_path.mkdir(parents=True, exist_ok=True)
        image_name = f"RW_States_J{J}_GP.png"
...
for lam_idx, lam in enumerate(true_lams.columns):
    ...
    if SAVE_PLOTS:
        folder_path = Path(PLOT_ROOT_FOLDER_NAME + "/"
                           + f"{EXAMPLE_FOLDER_NAME}/GP; J={J}")
        folder_path.mkdir(parents=True, exist_ok=True)
        image_name = f"Quantiles_{rate_name_img}_J{J}_GP.png"
        plt.savefig(folder_path / image_name,
                    bbox_inches='tight')
        plt.close("all")
    else:
